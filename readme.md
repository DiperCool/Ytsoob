...
[![Coverage Status](https://coveralls.io/repos/github/mehdihadeli/Ytsoob-microservices/badge.svg?branch=develop&style=flat-square)](https://coveralls.io/github/mehdihadeli/Ytsoob-microservices?branch=develop)
[![semantic-release](https://img.shields.io/badge/%20%20%F0%9F%93%A6%F0%9F%9A%80-semantic--release-e10079.svg?&style=flat-square)](https://github.com/semantic-release/semantic-release)
[![Commitizen friendly](https://img.shields.io/badge/commitizen-friendly-brightgreen.svg?&style=flat-square)](http://commitizen.github.io/cz-cli/)

[![Open in Gitpod](https://gitpod.io/button/open-in-gitpod.svg)](https://gitpod.io/https://github.com/mehdihadeli/Ytsoob-microservices)
...
> **Warning**
> This project is in progress. I add new features over the time. You can check the [Release Notes](https://github.com/mehdihadeli/Ytsoob-microservices/releases) and follow the progress on Twitter [@mehdi_hedli](https://twitter.com/shadcn) and Linkedin [mehdihadeli](https://www.linkedin.com/in/mehdihadeli/).

🎯 This Application ported to `modular monolith` approach in [Ytsoob-modular-monolith](https://github.com/mehdihadeli/Ytsoob-modular-monolith) repository, we can choose best fit architecture for our projects based on production needs.
...
| ---------------- | ------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| API Gateway      | Microsoft YARP Reverse Proxy          | Completed ✔️   | [![Gateway-CI-CD](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/gateway.yml/badge.svg?branch=develop&style=flat-square)](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/gateway.yml)              |
| Identity Service | Data Centeric Architecture (CRUD)     | Completed ✔️   | [![Identity-CI-CD](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/identity.yml/badge.svg?branch=develop&style=flat-square)](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/identity.yml)           |
| Customer Service | Domain Driven Design                  | Completed ✔️   | [![Customers-CI-CD](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/customers.yml/badge.svg?branch=develop&style=flat-square)](https://github.com/mehdihadeli/Ytsoob-microservices-sample/actions/workflows/customers.yml) |
| Catalog Service  | Domain Driven Design                  | Completed ✔️   | [![Catalogs-CI-CD](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/catalogs.yml/badge.svg?branch=develop&style=flat-square)](https://github.com/mehdihadeli/Ytsoob-microservices/actions/workflows/catalogs.yml)           |
...
`Ytsoob Microservices` is a simple online Ytsoob api sample that has the basic business scenario for online purchasing with some dedicated services. There are six possible `Bounded context` or `Service` for above business:
...
In this architecture we use [CQRS Pattern](https://www.eventYtsoob.com/cqrs-pattern) for separating read and write model beside of other [CQRS Advantages](https://youtu.be/dK4Yb6-LxAk?t=1029). Here for now I don't use [Event Sourcing](https://www.eventYtsoob.com/blog/event-sourcing-and-cqrs) for simplicity but I will use it in future for syncing read and write side with sending streams and using [Projection Feature](https://event-driven.io/en/how_to_do_events_projections_with_entity_framework/) for some subscribers to syncing their data through sent streams and creating our [Custom Read Models](https://codeopinion.com/projections-in-event-sourcing-build-any-model-you-want/) in subscribers side.
...
Also here I used [CQRS](https://www.eventYtsoob.com/cqrs-pattern) for decompose my features to very small parts that makes our application:
...
For implementing vertical slice architecture in each microservice, I have two projects, for example in `Catalog Service` I have [Ytsoob.Services.Catalogs](src/Services/Catalogs/Ytsoob.Services.Catalogs/) project and [Ytsoob.Services.Catalogs.Api](/src/Services/Catalogs/Ytsoob.Services.Catalogs.Api/) project.

- `Ytsoob.Services.Catalogs.Api` is responsible for Hosting microservice and configuring our `web api`, running the application on top of .net core and actually serving our microservices slices to outside of world.
- `Ytsoob.Services.Catalogs` is responsible for putting all slices (features) based on our functionality in some slices, for example we put all [Features or Slices](src/Services/Catalogs/Ytsoob.Services.Catalogs/Products/Features) related to `product` functionalities in [Products](src/Services/Ytsoob.Services.Catalogs/Ytsoob.Services.Catalogs/Products/) folder, also we have a [Shared Folder](src/Services/Catalogs/Ytsoob.Services.Catalogs/Shared) that contains some infrastructure things will share between all slices (for example [Data-Context](src/Services/Catalogs/Ytsoob.Services.Catalogs/Shared/Data/CatalogDbContext.cs), [ServiceCollectionExtensions.Persistence](src/Services/Catalogs/Ytsoob.Services.Catalogs/Shared/Extensions/ServiceCollectionExtensions/ServiceCollectionExtensions.Persistence.cs)).

In vertical slice flow, we treat each request as a `slice`. For example for [CreatingProduct](src/Services/Catalogs/Ytsoob.Services.Catalogs/Products/Features/CreatingProduct/) feature or slice, Our flow will start with a `Endpoint` with name [CreateProductEndpoint](src/Services/Catalogs/Ytsoob.Services.Catalogs/Products/Features/CreatingProduct/CreateProductEndpoint.cs) and inner this endpoint we handle the http request from out side of world and pass our request data with a mediator gateway to corresponding handler.
...
In this endpoint we use CQRS and pass [CreateProduct](src/Services/Catalogs/Ytsoob.Services.Catalogs/Products/Features/CreatingProduct/CreateProduct.cs) command to our command processor for executing and route to corresponding [CreateProductHandler](src/Services/Catalogs/Ytsoob.Services.Catalogs/Products/Features/CreatingProduct/CreateProduct.cs#L88) command handler.
...
7. Clone Project [https://github.com/mehdihadeli/Ytsoob-microservices-sample](https://github.com/mehdihadeli/Ytsoob-microservices-sample), make sure that's compiling
...
9. Open [Ytsoob.sln](./Ytsoob.sln) solution.
...
In this application I use a `fake email sender` with name of [ethereal](https://ethereal.email/) as a SMTP provider for sending email. after sending email by the application you can see the list of sent emails in [ethereal messages panel](https://ethereal.email/messages). My temp username and password is available inner the all of [appsettings file](./src/Services/Customers/Ytsoob.Services.Customers.Api/appsettings.json).
...
The project is under [MIT license](https://github.com/mehdihadeli/Ytsoob-microservices-sample/blob/main/LICENSE).

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=mehdihadeli/Ytsoob-microservices&type=Date)](https://star-history.com/#mehdihadeli/Ytsoob-microservices&Date)
