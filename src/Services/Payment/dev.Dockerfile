...
COPY ./src/Services/Customers/Ytsoob.Services.Payment/Ytsoob.Services.Payment.csproj ./Services/Customers/Ytsoob.Services.Payment/
COPY ./src/Services/Customers/Ytsoob.Services.Payment.Api/Ytsoob.Services.Payment.Api.csproj ./Services/Customers/Ytsoob.Services.Payment.Api/
...
# when installing packages, we don’t always need to fetch all of our packages from the internet each time. if we have any package update on `Ytsoob.Services.Payment.Api.csproj` this layer will rebuild but it don't download all packages again, it just download new packages and for exisitng one uses mount cache
RUN --mount=type=cache,id=customers_nuget,target=/root/.nuget/packages \
    dotnet restore ./Services/Customers/Ytsoob.Services.Payment.Api/Ytsoob.Services.Payment.Api.csproj

# Copy project files
COPY ./src/BuildingBlocks/ ./BuildingBlocks/
COPY ./src/Services/Customers/Ytsoob.Services.Payment.Api/  ./Services/Customers/Ytsoob.Services.Payment.Api/
COPY ./src/Services/Customers/Ytsoob.Services.Payment/  ./Services/Customers/Ytsoob.Services.Payment/
COPY ./src/Services/Shared/  ./Services/Shared/

WORKDIR /src/Services/Customers/Ytsoob.Services.Payment.Api/
...
ENTRYPOINT ["dotnet", "Ytsoob.Services.Payment.Api.dll"]
