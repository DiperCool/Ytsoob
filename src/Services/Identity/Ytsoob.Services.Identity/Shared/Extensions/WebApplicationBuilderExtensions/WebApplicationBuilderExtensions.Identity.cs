...
using Ytsoob.Services.Identity.Shared.Data;
    ...
    )
    {
        if (builder.Configuration.GetValue<bool>("PostgresOptions:UseInMemory"))
        {
            builder.Services.AddDbContext<IdentityContext>(
                options => options.UseInMemoryDatabase("Shop.Services.Ytsoob.Services.Identity")
            );
        ...
        // Problem with .net core identity - will override our default authentication scheme `JwtBearerDefaults.AuthenticationScheme` to unwanted `Ytsoob.Services.Identity.Application` in `AddIdentity()` method .net identity
