...
COPY ./src/Services/Identity/Ytsoob.Services.Identity/Ytsoob.Services.Identity.csproj ./Services/Identity/Ytsoob.Services.Identity/
COPY ./src/Services/Identity/Ytsoob.Services.Identity.Api/Ytsoob.Services.Identity.Api.csproj ./Services/Identity/Ytsoob.Services.Identity.Api/
...
# when installing packages, we don’t always need to fetch all of our packages from the internet each time. if we have any package update on `Ytsoob.Services.Identity.Api.csproj` this layer will rebuild but it don't download all packages again, it just download new packages and for exisitng one uses mount cache
RUN --mount=type=cache,id=identity_nuget,target=/root/.nuget/packages \
    dotnet restore ./Services/Identity/Ytsoob.Services.Identity.Api/Ytsoob.Services.Identity.Api.csproj

# Copy project files
COPY ./src/BuildingBlocks/ ./BuildingBlocks/
COPY ./src/Services/Identity/Ytsoob.Services.Identity.Api/  ./Services/Identity/Ytsoob.Services.Identity.Api/
COPY ./src/Services/Identity/Ytsoob.Services.Identity/  ./Services/Identity/Ytsoob.Services.Identity/
COPY ./src/Services/Shared/  ./Services/Shared/

WORKDIR /src/Services/Identity/Ytsoob.Services.Identity.Api/
...
ENTRYPOINT ["dotnet", "Ytsoob.Services.Identity.Api.dll"]
