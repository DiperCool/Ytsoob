...
COPY ./src/ApiGateway/Ytsoob.ApiGateway/Ytsoob.ApiGateway.csproj ./ApiGateway/Ytsoob.ApiGateway/
...
# when installing packages, we don’t always need to fetch all of our packages from the internet each time. if we have any package update on `Ytsoob.ApiGateway.csproj` this layer will rebuild but it don't download all packages again, it just download new packages and for exisitng one uses mount cache
RUN --mount=type=cache,id=gateway_nuget,target=/root/.nuget/packages \
    dotnet restore ./ApiGateway/Ytsoob.ApiGateway/Ytsoob.ApiGateway.csproj

# Copy project files
COPY ./src/BuildingBlocks/ ./BuildingBlocks/
COPY ./src/ApiGateway/Ytsoob.ApiGateway/  ./ApiGateway/Ytsoob.ApiGateway/

WORKDIR /src/ApiGateway/Ytsoob.ApiGateway/
...
ENTRYPOINT ["dotnet", "Ytsoob.ApiGateway.dll"]
